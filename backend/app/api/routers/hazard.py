...
async def add_hazard_zone(zone: HazardZoneAdd):
    ...
    st.static_zones.append(new_zone)
    ...
    return {"status": "added", "zone_id": zone.id, "total_zones": len(st.static_zones)}
...
async def add_sensor_reading(reading: SensorReadingAdd):
    ...
    return {"status": "added", "total_readings": len(st.sensor_readings)}
...
async def fuse_hazards():
    ...
    if not st.graph_data or not st.graph_data.habitations:
        raise HTTPException(400, "No habitations loaded.")

    now = datetime.utcnow()
    results = {}
    for hab in st.graph_data.habitations:
        rel_sensors = [s for s in st.sensor_readings if st.haversine(hab.location, s.location) <= 10.0]
        rel_reports = [r for r in st.crowd_reports if st.haversine(hab.location, r.location) <= 10.0]
        for htype in [HazardType.FLOOD, HazardType.LANDSLIDE, HazardType.SEISMIC]:
            ...
            st.hazard_confidences[key] = score
            results[key] = {
